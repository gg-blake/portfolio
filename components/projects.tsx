...
function ProjectItem({ date, title, desc, href, location, tags }: ProjectEntry) {
    ...
    return (
        <div className='w-[400px] overflow-y-auto h-[380px] border-[1px] box-border relative border-white p-3'>
...
function YearContainer({ children, year, order, length }: {children?: any, year: string, order: number, length: number }) {
    ...
    const scrollIntoView = () => {
        ...
        const projectContainerRef = thisRef.current.parentElement;
        projectContainerRef.scroll({top: thisRef.current.getBoundingClientRect().top - 88, behavior: "smooth"});
    }

    return (
        <>
        <div id={`project-year-${year}`} ref={thisRef} style={{top: `${(order)*84}px`}} className="flex flex-col h-[500px] bg-black sticky box-border w-full border-[1px] overflow-y-clip border-white p-3 transition-[height] duration-500">
            <h1 onClick={() => scrollIntoView()} className="p-0 m-0 hover:underline active:no-underline text-[40px] h-auto font-semibold w-full">{year}</h1>
...
export default function Projects() {
    ...
    function HiddenProjectItem({ year }: { year: string }) {
        ...
        const scrollToById = (id: string) => {
            ...
            if (!element) return;
            containerRef.current.scroll({top: element.getBoundingClientRect().top - 88, behavior: "smooth"});
        }

        return (
            <div onClick={() => scrollToById(`project-year-${year}`)} className="h-[86px] w-full border-white border-[1px] text-[40px] bg-black p-3 font-semibold hover:underline active:no-underline">{year}</div>
        )
    }
